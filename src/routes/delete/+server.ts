...
export const POST: RequestHandler = async ({ url, request }) => {
    ...
    if (thread_data.gtripcode != tripcode(tripcode_password) && tripcode_password != ADMIN_PASSWORD) { throw redirect(303, '/') }
    ...
    if (image_deletion_error) { console.error(image_deletion_error.message) }
    const { data: posts_data, error: posts_error } = await supabase.from('posts').delete().eq('parent_thread_id', id);
    if (posts_error) { console.error(posts_error.message) }
